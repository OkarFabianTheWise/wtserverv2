 ...
 */
export async function renderAnimationScriptToVideo(
 ...
 * @returns Buffer containing the final video with audio
 */
export async function generateVideoWithRemotion2(
    text: string,
    audioBuffer?: Buffer,
    outputPath?: string
): Promise<Buffer> {
    ...
    try {
        ...
        const animationScript: AnimationScript = await processCodeWithOpenAI(text);
