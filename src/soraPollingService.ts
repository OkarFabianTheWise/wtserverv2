 ...
 */
export function setWebSocketManager(wsManager: any) {
 ...
 */
async function getPendingJobs(): Promise<PendingSoraJob[]> {
 ...
 */
async function pollSoraJob(job: PendingSoraJob) {
    try {
        ...
        if (VERBOSE && statusChanged) {
            console.log(
                `   Status: ${job.sora_status} → ${newStatus} (Progress: ${job.sora_progress}% → ${newProgress}%)`
            );
        }
        ...
        if ((statusChanged || progressChanged) && webSocketManager) {
            webSocketManager.emitProgress(
                ...
                newProgress,
                newStatus,
                `Processing... ${newProgress}%`,
                { sora_status: newStatus }
            );
        }
        ...
        if (statusChanged || progressChanged) {
            if (job.webhook_url) {
                ...
                if (VERBOSE) console.log(`   ⚠️  No webhook URL for job ${job.job_id}`);
            }
        }
        ...
        if (newStatus === 'completed') {
            ...
            const errorMsg = (video as any).error?.message || 'Unknown error';
            console.log(`❌ Sora job failed: ${job.sora_job_id} - ${errorMsg}`);
        }
    } catch (error) {
        console.error(
            `❌ Error polling Sora job ${job.sora_job_id}:`,
 ...
 */
async function updateJobStatus(jobId: string, soraStatus: string, progress: number) {
...
async function sendWebhook(
    ...
    try {
        ...
        if (response.ok) {
            if (VERBOSE) console.log(`   ✅ Webhook sent: ${job.webhook_url}`);
        } else {
            console.warn(
                `   ⚠️  Webhook failed (${response.status}): ${job.webhook_url}`
            );
        }
    } catch (error) {
        console.error(
            `   ❌ Error sending webhook to ${job.webhook_url}:`,
            error instanceof Error ? error.message : error
        );
    }
}
 ...
 */
export async function registerSoraJob(
    jobId: string,
    soraJobId: string,
    webhookUrl?: string
) {
 ...
 */
export async function forcePollJob(jobId: string) {
    try {
        // Get sora_job_id from database
        const jobs = await getPendingJobs();
        const job = jobs.find((j: PendingSoraJob) => j.job_id === jobId);

        if (!job || !job.sora_job_id) {
            throw new Error('Job not found or not a Sora job');
        }

        const video = await (openai as any).videos.retrieve(job.sora_job_id);
