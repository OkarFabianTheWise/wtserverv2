...
import pool, { testConnection, getVideoByJobId, getVideoByVideoId, getVideosByWallet, getAudioByJobId, getAudioByAudioId, getContentByWallet, getCompletedJobsCount, getTotalDurationSecondsForWallet, getTotalUsersCount, getTotalVideosCreated, getTotalFailedJobs, updateJobStatus, storeVideo, ensureUser, getVideoBuffer } from './db.js';
...
app.get('/api/openai/videos/:videoId', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to fetch video from OpenAI: ' + err.message });
    }
});
...
app.get('/api/openai/videos', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to fetch videos from OpenAI: ' + err.message });
    }
});
...
app.get('/api/openai/videos/:videoId/download', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to download video from OpenAI: ' + err.message });
    }
});
...
app.post('/api/webhooks/job-update', async (req, res) => {
    try {
        ...
        const jobResult = await pool.query('SELECT sora_job_id FROM video_jobs WHERE job_id = $1', [jobId]);
        ...
        if (isSoraJob && status === 'completed') {
            ...
            try {
                ...
                const result = await pool.query('SELECT wallet_address, video_buffer, video_size FROM video_jobs WHERE job_id = $1', [jobId]);
